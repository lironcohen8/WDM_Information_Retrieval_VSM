        ...
        for record in records:
            ...
            for word in record_words:
                ...
                if clean_word not in set(stopwords.words('english')):
                    ...
                    max_freq = max(max_freq, words_dict[clean_word]["docs"][record_num]["words_cnt"])
...
def update_words_dict(clean_word, record_num):
    ...
    if record_num not in words_dict[clean_word]["docs"].keys():
        words_dict[clean_word]["docs"][record_num] = {"words_cnt": 0, "tf": 0}
    words_dict[clean_word]["docs"][record_num]["words_cnt"] += 1
...
def calc_tf_values(record_num, max_freq):
    for word in words_dict.keys():
        ...
        if record_num in words_dict[word]["docs"].keys():
            tf = words_dict[word]["docs"][record_num]["words_cnt"] / max_freq
...
def calc_tfidf_grades(query_dict):
    ...
    relevant_records_list = [record for record in relevant_records.items() if record[1] > SCORELIMIT]
    sorted_records = sorted(relevant_records_list, key=lambda x: x[1], reverse=True)
